...
| Dockyard support | The original had no way to recover damage, so a long war only ever ran down. In an extended war a hull that ends a stardate inside `DOCKING.range` (8) of an active friendly starbase — one still holding at least half its own shields — and is not tractor-held, recovers 8% of its shield capacity and 4 crew, each capped at the ship's complement. Subsystem units are never restored. It resolves in the computer phase, is narrated, and is drawn as a green ring on the map. |
| Battle report | Ending the war adds a report to the concluded panel: stardates elapsed, Federation losses, top gun, heaviest punishment taken, most collisions, and the player's own record. Collisions are now counted per hull (`collisions`), which is the only new per-ship field. |
| Alliance doctrines | The original ran every autopilot on one behavior: pursue the fleet's shared target, fire, and never mind its own skin | Watch an alliance fight in an extended war | Each alliance fights its own way, and only on hulls the player has not ordered — an order always outranks doctrine. **Axis** swarms the nearest hull at 6 units, refits below 20% shields, gives ground only below 8%, and self-destructs below 8% when at least four enemies — and more enemies than friends — sit inside its blast. **Bloc** works the phaser edge at 18, backs off anything inside 5, never tractors, and concentrates on the weakest hull it can actually hit. **Cabal** concentrates with the fleet but spends a tractor beam whenever the tow would smash the target into another enemy, never onto one of its own. **Federation** keeps the original's fleet concentration and adds damage discipline: refit below 35%, withdraw below 12%. |
...
| Doctrine balance | — | Simulate 40 whole wars per configuration | Tuned against a full-war simulation so no alliance dominates. Over 40 seeds a classic war went Federation 13 / Axis 12 / Bloc 10 / Cabal 5 at a mean 15.7 stardates; an extended war with no player orders went Axis 16 / Federation 13 / Bloc 8 / Cabal 3 at 18.5. Ordering three cruisers to screen Xanadu lifts the Federation to 15, and mass-intercepting the Axis flagship hands the war to Bloc — both are intended consequences of the dockyard and of removing one alliance early. |
...
`DOCKING`, `STALEMATE_ROUNDS`, `PERSONALITIES`) instead of scattered through the
modules, so retuning a row here means editing one file.
