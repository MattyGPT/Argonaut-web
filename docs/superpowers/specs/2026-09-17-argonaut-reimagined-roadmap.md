...
### Phase 6 — The sector campaign *(capstone; #12)* — ✅ COMPLETE (26a #64, 26b #65, 26c #66, mix retune #67, 27a #68, 27b #69)
...
### Phase 7 — Meta & community *(#14)* — DEFERRED (Matt, 2026-09-25)

**Deferred by Matt on 2026-09-25 in favor of Phase 8 (real-time) and the new
Phase 9 (ship sprites)** — parked, not dropped, exactly like round 25 and the
mines. When it is picked up, rounds 28–29 below stand as written; note that
Phase 8's determinism work (a fixed-timestep headless sim core) is what makes
reproducible seed challenges cheap, so 28 arguably gets easier after 30–32.
...
artillery, carrier) — and wants the **modern view** to grow past letter glyphs
into real graphics. Sequenced **after Phase 8** so the sprite layer lands on a
continuous-motion field where heading is already a first-class visual (the
sprites rotate to face). The **classic view stays letters and phosphor** — it
is the 1992 reading of the game, and the parity story is untouched either way.
Gets its own design doc with its own open questions for Matt when picked up.
Provisional chunks, to be re-cut by that doc:
...
`bloc-fleet-sheet.jpeg` (purple, bows up), `cabal-fleet-sheet.jpeg` (gold),
each six classes, generated by Matt (Gemini) as concept art on 2026-09-25.

Design notes for the doc:

- **Orientation is not consistent across sheets** — Bloc bows point up, the
  other three point right — while `ship.facing` is degrees with 0 = +x,
  clockwise on the y-down field. The pipeline must normalize a bow direction
  per sprite (a rotation offset per sheet, or re-authored art) before heading
  rotation can drive it.
- **The sheets are JPEGs on mixed backgrounds** (black, white, panelled);
  pixel art needs alpha. Expect a re-authoring or careful keying step to
  transparent PNGs, ideally at a small native size (32–64px) scaled crisp
  (`image-rendering: pixelated`), not the 2048px concept scale.
- **Identity vs alliance color:** the game reads alliance from glyph color
  today; sprites carry their own palette (the sheets already do: Fed
  white/blue, Axis red, Bloc purple, Cabal gold — close to but not exactly
  `--fed/--axis/--bloc/--cabal`). Decide whether the faction ring/outline
  carries identity and the sprite carries detail, or the palettes converge.
- **Everything that stacks on a glyph today must survive**: heading needle,
  stance halo, threat outline, order/ace/prize/tractor pips, distress blinker,
  the stack-declutter fan, fog-of-war absence, wrecks, and the minimap dots.
  The sprite is a replacement for the disc+letter, not for the marker layer.
- **Provenance:** the sheets are AI-generated concept art commissioned by Matt
  for this project; the shipped sprites derived from them are this remake's
  own expression, which continues to widen its distance from the 1992
  original (the README rights posture benefits, as with every original asset).
...
| Campaign play-test pass (27c, 2026-09-25) | Matt's campaign play-test: too many collisions, and stacked hulls are hard to see or click. Measured first: collisions/war (new harness metric) 20.4 at 240 and *worse* on wider fields (22.1 at 320, 22.8 at 400 — engine capacity scales with the field), 44% of them drones converging on one integer point, 43% Cabal tow-rams. Shipped: field 240 → **320** (Matt's pick for strategic room), **drone arrival avoidance** (Reimagined autopilot drones nudge to the nearest free point; pursuit clumsiness, tow-rams, hyperspace, and player rams stay), last-stand gate for the denser clusters (`REIMAGINED_SELF_DESTRUCT_SCALE` 0.35, `REIMAGINED_SUICIDE_MIN_ENEMIES` 7), camera tightest view held at ~40 units on any field (`maxZoomFor`), and same-point glyphs fan onto a screen-space ring so every hull is seeable and clickable | Sim: collisions 13.5/war (−39%), median 55, draws 12%, 4+-hull blasts 4.4%, prizes 10.0, no runaway winner; classic/extended digit-for-digit | ✅ PR #70 |
